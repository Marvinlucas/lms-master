...
class DepartmentController extends Controller
{
    ...
    public function store(DepartmentRequest $request)
    {
        ...
        if ($request->hasFile('image')) {
            $imagePath = $request->file('image')->store('public/departments');
        } else {
            $imagePath = null;
        }
    ...
    public function update(DepartmentRequest $request, int $department_id)
    {
        ...
        $requestData = $request->except('image');
        $requestData['image'] = $imagePath;
        $this->service->update($requestData, $department_id);
    ...
    public function destroy(int $department_id)
    {
        ...
        if ($this->service->destroy($department_id)) {
            ...
            return redirect()->back()->with('error', 'Cannot delete this item because it is associated with other records.');
        }
    }
}
