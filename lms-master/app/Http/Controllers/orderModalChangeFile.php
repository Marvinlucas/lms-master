...
class orderModalChangeFile extends Controller
{
    use Sequence;
     ...
     * @return \Illuminate\Routing\Redirector|\Illuminate\Http\RedirectResponse
     */
    public function changeModalOrderFile(DocumentFile $from, $move)
    {
        ...
        return redirect()->back()->with('showModal1', $from->document_id);
     ...
     * @return \Illuminate\Routing\Redirector|\Illuminate\Http\RedirectResponse
     */
    public function addModalFileToDocument(Document $document, File $file)
    {
        ...
        // Set the session variable with the document ID
        return redirect()->back()->with('showModal1', $document->id);
    }
     ...
     * @return \Illuminate\Routing\Redirector|\Illuminate\Http\RedirectResponse
     */
    public function deleteDocsDocument(DocumentFile $documentFile)
{
    $documentId = $documentFile->document_id; // Get the document ID before deleting

    $documentFile->delete();

    // Set the session variable with the document ID
    return redirect()->back()->with('showModal1', $documentId);
}
